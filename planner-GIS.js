...
async function initializeMapPlannersWithNewFeatures(randomGeoJSON) {
  try {
    ...
    intl.setLocale("ar");
    esriConfig.apiKey = "AAPKd0a4217e82944515a7f3df52cb9d578cg-xi22yCw4m8d85NYxlC4d7q_58JlYYlu0c2GBPsXinGmbvN1wB5-86rp39u1qWn";
...
async function createLayers(geojsons, randomGeoJSON) {
  try {
    const [GeoJSONLayer, FeatureLayer, Query, reactiveUtils, Legend, FeatureTable] =
      await Promise.all([
    ...
    const renderer = {
      ...
      legendOptions: {
        title: "تصنيف المخطط تبعا لاستخدام الأرض",
      },
      // defaultSymbol: {
      //   type: "simple-fill", // autocasts as new SimpleFillSymbol()
      //   color: "#38689a",
    ...
    await createFeatureLayers(geojsons).then((featureLayers) => {
      ...
      if (geojsons.length === 1) {
        ...
        if (featureLayers) {
          ...
          const getAllLayerViews = view.map.allLayers.map(layer => view.whenLayerView(layer));
